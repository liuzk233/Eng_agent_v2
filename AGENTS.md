...
- **R4.5** `.specs/` 目录不得提交到 Git（已在 `.gitignore` 中排除）
...
- **R5.4 · 登录凭据** 需要登录的 UAT / playwright-cli / E2E 操作统一从仓库根 `.env` 读取 `E2E_TEST_EMAIL` 与 `E2E_TEST_PASSWORD`；禁止在 `TEST.md` / `UAT.md` / `REVIEW.md` / `SUMMARY.md` 中记录明文密码，产物只记录变量名、登录结果与证据路径
...
- **R8.1** 工件 `.md` 与项目主语言一致
- **R8.2** 代码标识符英文，注释允许中文
- **R8.3** 后续读取 `flow-kit/*.md` 时默认使用 UTF-8（PowerShell 使用 `Get-Content -Encoding UTF8`），避免乱码影响判断
